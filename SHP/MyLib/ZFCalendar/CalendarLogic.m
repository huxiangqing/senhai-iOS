...
//从过去到现在的月数组（正数是之前几天，负数是之后的几天）
- (NSMutableArray *)reloadCalendarViewWithTheEndDate:(NSDate *)date  selectDate:(NSDate *)selectdate needDays:(int)days_number
{
    ...
    if (self.timeInterval) {
        ...
        start=[NSDate dateWithTimeIntervalSince1970:[self.timeInterval doubleValue]/1000.0];
    }
    else
    {
        start = [date dateFromString:@"2015.1.1"];//计算它days天以后的时间
    }
    ...
    NSDateComponents *startDC= [start YMDComponents];
    
    NSInteger todayYear = todayDC.year;
    
    NSInteger todayMonth = todayDC.month;
    
    NSInteger startYear = startDC.year;
    
    NSInteger startMonth = startDC.month;
    
    NSInteger months = (todayYear - startYear) * 12 + (todayMonth - startMonth);
    
    NSMutableArray *calendarMonth = [[NSMutableArray alloc]init];//每个月的dayModel数组
    
    for (int i = 0; i <= months; i++) {
        
        NSDate *month = [start dayInTheFollowingMonth:i];
        ...
        [self calculateDaysInCurrentMonthWithDateFromPassedToNow:month andArray:calendarDays];
...
#pragma mark - 可选不可选
//计算当月的天数
- (void)calculateDaysInCurrentMonthWithDateFromPassedToNow:(NSDate *)date andArray:(NSMutableArray *)array
{
    ...
    for (int i = 1; i < daysCount + 1; ++i) {
        ...
        [self changPassedStyle:calendarDay];
        [array addObject:calendarDay];
    }
}
- (void)changPassedStyle:(CalendarDayModel *)calendarDay
{
    
    NSDateComponents *calendarToDay  = [today YMDComponents];//今天
    NSDateComponents *calendarstart = [start YMDComponents];//最开始那天
    ...
    if(calendarSelect.year == calendarDay.year &
       ...
       calendarSelect.day == calendarDay.day){
        ...
        if (calendarToDay.year == calendarDay.year & calendarToDay.month == calendarDay.month &
    ...
    [self CalendarDayModelSetHoliday:calendarDay calendarToDay:calendarToDay];
    
    
}


-(void)CalendarDayModelSetHoliday:(CalendarDayModel *)calendarDay calendarToDay:(NSDateComponents *)calendarToDay
{
    //===================================
    //这里来判断节日
    //今天
