...
- (void)setHotelToDay:(int)day ToDateforString:(NSString *)todate
{
    ...
    NSIndexPath *index = [NSIndexPath indexPathForRow:4 inSection:super.calendarMonth.count - 1];
    [super.collectionView scrollToItemAtIndexPath:index atScrollPosition:UICollectionViewScrollPositionTop animated:YES];
...
//酒店初始化方法
- (void)setPassedToDay:(int)day ToDateforString:(NSString *)todate
{
    
    daynumber = day;
    optiondaynumber = 2;//选择两个后返回数据对象
    super.calendarMonth = [self getMonthArrayOfPassedDayNumber:daynumber ToDateforString:todate];
...
#pragma mark - 逻辑代码初始化
//获取过去时间段内的天数数组
- (NSMutableArray *)getMonthArrayOfPassedDayNumber:(int)day ToDateforString:(NSString *)todate
{
    ...
    super.Logic.timeInterval = self.timeInterval;
    
    return [super.Logic reloadCalendarViewWithTheEndDate:date selectDate:date needDays:day];
}
//获取时间段内的天数数组
