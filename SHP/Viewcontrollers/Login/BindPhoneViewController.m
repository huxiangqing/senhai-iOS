...
-(void)createDataMessagePhoneNum:(NSString *)phoneNumStr
{
    ...
    [apis GetSendMessagePhoneNum:phoneNumStr type:@"2" Completion:^(id objectRet, NSError *errorRes)
     {
         if (objectRet)
         {
             ...
             if ([statusStr isEqualToString:@"1"])
             {
                 ...
                 _userPhone = [NSString stringWithFormat:@"%@",[[objectRet objectForKey:@"data"]objectForKey:@"userPhone"]];
                 //发送时间
...
- (void)registerInformation:(UIButton *)sender
{
    ...
    {
        if ([__weakObject.userPhone isEqualToString:@"1"]) {
            
            LLNetApiBase *apis =[[LLNetApiBase alloc]init];
            
            [apis PostUserInfoRegisterUserName:__weakObject.thirdPartUID phone:_phoneText_UITextField.text userType:@"1" realName:@"" sex:@"" birthday:@"" height:@"" weight:@"" weiChatOrWeiBO:__weakObject.weiChatOrWeiBO info:__weakObject.info andCompletion:^(id objectRet, NSError *errorRes)
             {
                 if (objectRet)
                 {
                     NSString *statusStr =[NSString stringWithFormat:@"%@",[objectRet objectForKey:@"status"]];
            ...
            [apis POSTCheckVerificationCodeTelephone:_phoneText_UITextField.text verificationCode:_codeText_UITextField.text sendTime:_sendTimeStr andCompletion:^(id objectRet, NSError *errorRes)
             {
                 if (objectRet)
                 {
                     ...
                     if ([statusStr isEqualToString:@"1"])
                     {
                         ...
                         editinfo.weiChatOrWeiBO = __weakObject.weiChatOrWeiBO;
                         editinfo.info = __weakObject.info;
    ...
    {
        [SVProgressHUD showInfoWithStatus:NSLocalizedString(@"Please enter the correct verification code", nil)];
    }
}
